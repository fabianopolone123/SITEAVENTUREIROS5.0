...
- Ajustei os textarea para altura m�nima consistente e desativei o redimensionamento, mantendo o layout intacto mesmo com o cursor pr�ximo � borda.
...
- Layout permanece est�vel com colunas regulares e os blocos n�o saltam quando o textarea recebe foco.
- O formul�rio agora impede qualquer redimensionamento (horizontal ou vertical), eliminando o comportamento estranho de aumentar o campo ao arrastar o canto.
---
...
#### ?? Contexto
> O layout da ficha m�dica ainda parecia �quadrado demais� e os campos n�o estavam alinhados com a proposta retangular moderna das demais se��es, ent�o o usu�rio pediu redistribui��o e posicionamento mais limpo.

#### ??? O que foi feito
- Refinei `.ficha-card` para que cada bloco de campos use `display: grid` com `gap` consistente e cada item receba um cart�o retangular leve com bordas suaves, sombra e padding uniforme.
- Apliquei estilos espec�ficos aos `label` e aos inputs dentro da ficha m�dica, deixando-os mai�sculos, mais estreitos e com fundo uniforme escuro-claro para refor�ar o formato retangular e o foco visual.
- Padronizei os campos (`input`, `select`, `textarea`) dentro da ficha m�dica para que mantenham o mesmo preenchimento e sombra, fortalecendo o alinhamento e a sensa��o de estrutura.
...
#### ?? Impacto / Observa��es
- Cada campo agora parece um cart�o retangular com espa�amento homog�neo e a ficha m�dica combina visualmente com os outros cards do wizard.
- A leitura dos r�tulos e a navega��o pelos campos ficaram mais claras sem alterar a l�gica funcional do formul�rio.
---
...
#### ?? Contexto
> O usu�rio pediu uma reconstru��o total da ficha m�dica: campos retangulares, melhor distribui��o e grid consistente com os demais cards do wizard.

#### ??? O que foi feito
- Reescrevi `backend/ui/templates/cadastro_aventureiro/ficha.html` para encerrar cada campo em `<div class="ficha-field">` e usei varia��es de `.ficha-fields` (`--single`, `--grid`, `--checkbox`) para controlar a dispers�o e o comportamento dos campos.
- Criei os estilos `.ficha-fields`, `.ficha-field`, `.ficha-field-input` e `.ficha-field--checkbox` em `backend/ui/static/css/cadastro.css`, definindo cart�es retangulares com bordas suaves, sombra, padding uniforme e labels em caixa alta.
- Ajustei os inputs/selects/textarea da ficha m�dica para manter o mesmo padding e fundo, al�m de garantir que `textarea` mantenha altura fixa e que os cart�es respeitem um gap consistente para preservar a estrutura retangular.
...
#### ?? Relacionado a
- refactor: layout completo da ficha m�dica

#### ?? Impacto / Observa��es
- A ficha m�dica agora se comporta como cart�es retangulares alinhados, com espa�amento homog�neo e visual coerente com todo o wizard.
- A usabilidade melhorou sem alterar os fluxos e valida��es existentes; basta navegar pela p�gina para perceber o novo espa�o visual.
